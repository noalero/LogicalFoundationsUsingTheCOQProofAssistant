...
(** **** Exercise: 3 stars, standard, optional (optimize_0plus_b_sound)
...
Proof.
  ...
  reflexivity); try(simpl; rewrite IHb; reflexivity); 
  try(simpl; rewrite IHb1; rewrite IHb2; reflexivity).
  Qed.
  
  (*   - (* BTrue *) simpl. reflexivity.
  - (* BFalse *) simpl. reflexivity.
  - (* BEq *) simpl. repeat( rewrite optimize_0plus_sound''). reflexivity.
  - (* BNeq *) simpl. repeat( rewrite optimize_0plus_sound''). reflexivity.
  - (* BLe *) simpl. repeat( rewrite optimize_0plus_sound''). reflexivity.
  - (* BGt *) simpl. repeat( rewrite optimize_0plus_sound''). reflexivity.
...
(** **** Exercise: 3 stars, standard, optional (bevalR)
...
Inductive bevalR: bexp -> bool -> Prop :=
  ...
  | E_BNot (e: bexp) (b: bool) : (BNot e) ==>b negb b
...
Theorem XtimesYinZ_spec : forall st x y st',
  ...
  st' Z = x * y.
Proof.
  intros st x y st' Ex Ey E_times.
  inversion E_times. subst. simpl. apply t_update_eq.
Qed.
...
Inductive no_whilesR: com -> Prop := 
 ...
 | NW_Asgn : forall x n, (no_whilesR <{x := n}>)
...
Proof.
   ...
   - (* ->*)
     { induction c.                
       + (* skip *) intros H. apply NW_Skip.
       + (* asign *) intros H. apply NW_Asgn.
       + (* seq *) intros H. inversion H. apply andb_prop in H1. apply NW_Seq.
         * apply IHc1. apply H1.
         * apply IHc2. apply H1.
       + (* if *) intros H. inversion H. apply andb_prop in H1. apply NW_If.
         * apply IHc1. apply H1.
         * apply IHc2. apply H1.
       + (* while *) intros H. discriminate.
     }
   - (* <- *)
     { induction c; intros H; try(reflexivity).
       + (* seq *) inversion H. apply IHc1 in H2. apply IHc2 in H3.
         simpl. rewrite H2. rewrite H3. reflexivity.
       + (* if *) inversion H. apply IHc1 in H2. apply IHc2 in H4.
         simpl. rewrite H2. rewrite H4. reflexivity.
       + (* while *) inversion H.
     }
Qed.
(** [] *)
...
Proof.
  ...
  - (* skip *) intros st. exists st. constructor.
  - (* asign *) intros st. exists (x !-> (aeval st a) ; st). constructor. reflexivity.
  - (* seq *) intros st. inversion H. subst. 
    assert (Hc1: exists st', st =[c1]=> st'). { apply IHc1. apply H2. } inversion Hc1.
    assert (Hc2: exists st'', x =[c2]=> st''). { apply IHc2. apply H3. } inversion Hc2.
    exists x0.
    apply E_Seq with (c1 := c1) (c2 := c2) (st := st) (st' := x) (st'' := x0).
    + assumption.
    + assumption.
  - (* if *) intros st. inversion H. subst.
    assert (Hc1: exists st1, st =[c1]=> st1). { apply IHc1. apply H2. }
    assert (Hc2: exists st2, st =[c2]=> st2). { apply IHc2. apply H4. }
    remember (beval st b) as v. destruct v.    
    (* We want to destruct the boolean value of b, not the bexp *)
    + (* true *) inversion Hc1. exists x. apply E_IfTrue.
      symmetry. assumption. assumption.
    + (* false *) inversion Hc2. exists x. apply E_IfFalse.
      symmetry. assumption. assumption.
  - (* while *) rewrite <- no_whiles_eqv in H. discriminate.
Qed.
...
Fixpoint s_execute (st : state) (stack : list nat)
  ...
  | [], _ => stack
  | (SPush n) :: p, _ => (s_execute st (n::stack) p)
  | (SLoad x) :: p, _ => (s_execute st ((st x)::stack) p)
  | SPlus :: p, [] => (s_execute st stack p)
  | SPlus :: p, a::[] => (s_execute st stack p)
  | SPlus :: p, a1 :: (a2 :: s) => (s_execute st ((a2 + a1)::s) p)
  | SMinus :: p, [] => (s_execute st stack p)
  | SMinus :: p,  a::[] => (s_execute st stack p)
  | SMinus :: p, a1 :: (a2 :: s) => (s_execute st ((a2 - a1)::s) p)
  | SMult :: p, [] => (s_execute st stack p)
  | SMult :: p, a::[] => (s_execute st stack p)
  | SMult :: p, a1 :: (a2 :: s) => (s_execute st ((a2 * a1)::s) p)
   end.
...
Proof.
  intros st p1 p2. induction p1 as [| h1 l1].
  - (* [] *) simpl. reflexivity. 
  - (* h1::l1 *) destruct h1 eqn: Eh1; 
                    try (intros stack; simpl; apply IHl1);
                     destruct stack as [| s stack'] eqn: Estack;
                  try(simpl; apply IHl1); try(destruct stack' as [| s' stack''] eqn: Es';
                          simpl; apply IHl1).
Qed.
...
Proof.
  ...
  try(intros stack; simpl; repeat(rewrite execute_app);
  rewrite IHe2; rewrite IHe1; simpl; reflexivity).
Qed.
...
Inductive com : Type :=
  ...
  | CIf (b : bexp) (c1 c2 : com)
  | CWhile (b : bexp) (c : com)
  | CFor (init next body: com) (test: bexp).
...
Inductive ceval : com -> state -> result -> state -> Prop :=
  ...
  | E_Seq_Break : forall c1 c2 st st',
                  st  =[ c1 ]=> st' / SBreak ->
                  st  =[ c1 ; c2 ]=> st' / SBreak
  | E_Seq_Continue : forall c1 c2 st st' st'' S2,
                     st  =[ c1 ]=> st' / SContinue ->
                     st' =[ c2 ]=> st'' / (S2: result) ->
                     st  =[ c1 ; c2 ]=> st'' / S2
  | E_IfTrue : forall st st' b c1 c2 S1,
              beval st b = true ->
              st =[ c1 ]=> st' / S1 ->
              st =[ if b then c1 else c2 end]=> st' / S1
  | E_IfFalse : forall st st' b c1 c2 S2,
              beval st b = false ->
              st =[ c2 ]=> st' / S2 ->
              st =[ if b then c1 else c2 end]=> st' / S2
  | E_WhileFalse : forall b st c,
                  beval st b = false ->
  ...
  | E_WhileTrue_Break : forall st st' b c,
                        ...
                        st  =[ c ]=> st' / SBreak ->
                        st  =[ while b do c end ]=> st' / SContinue
  | E_For_Init_Break : forall st st' init next body test,
                       st =[ init ]=> st' / SBreak -> 
                       st =[ CFor init next body test ]=> st' / SContinue
  | E_ForFalse : forall st st' init next body test,
                 st =[ init ]=> st' / SContinue -> 
                 beval st' test = false ->
                 st =[ CFor init next body test ]=> st' / SContinue
  | E_ForTrue_Break1 : forall st st' st'' init next body test,
                      st =[ init ]=> st' / SContinue ->
                      beval st' test = true ->
                      st' =[ body ]=> st'' / SBreak ->
                      st =[ CFor init next body test ]=> st'' / SContinue
  | E_ForTrue_Break2 : forall st st' st'' st''' init next body test,
                      st =[ init ]=> st' / SContinue ->
                      beval st' test = true ->
                      st' =[ body ]=> st'' / SContinue ->
                      st'' =[ next ]=> st''' / SBreak ->
                      st =[ CFor init next body test ]=> st''' / SContinue
  | E_ForTrue_Continue : forall st st' st'' st''' init next body test,
                         st =[ init ]=> st' / SContinue ->
                         beval st' test = true ->
                         st' =[ body ]=> st'' / SContinue ->
                         st'' =[ next ]=> st''' / SContinue ->
                         st =[ CFor init next body test ]=> st''' / SContinue
...
Theorem break_ignore : forall c st st' s,
     ...
     st = st'.
Proof.
  intros c st st' s H. inversion H. subst. inversion H5.
   reflexivity. subst. inversion H2.
Qed.  
...
Theorem while_stops_on_break : forall b c st st',
  ...
  st =[ while b do c end ]=> st' / SContinue.
Proof.
  intros b c st st' H. apply E_WhileTrue_Break. assumption.
Qed.
...
Proof.
  intros c1 c2 st st' st''. apply E_Seq_Continue. Qed.

Theorem seq_stops_on_break : forall c1 c2 st st',
  st =[ c1 ]=> st' / SBreak ->
  st =[ c1 ; c2 ]=> st' / SBreak.
Proof.
  intros c1 c2 st st'. apply E_Seq_Break. Qed.
(** [] *)
...
Proof.
  intros b c st st' Hwhile Htrue. remember (CWhile b c) as wh.
  induction Hwhile; inversion Heqwh; subst.
  - (* E_WhileFalse *) rewrite H in Htrue. discriminate.
  - (* E_WhileTrue_Continue *) apply IHHwhile2. reflexivity. assumption.
  - (* E_WhileTrue_Break *) exists st. apply Hwhile.
Qed.
(** [] *)
...
Proof.
  ...
  induction H1; intros s2 st2 H2; inversion H2; subst;
  try(split; reflexivity); try(auto); try(rewrite H8 in H; discriminate);
   try(rewrite H3 in H; discriminate); try (apply IHceval; assumption);
   try(try(rewrite H6 in H); try(rewrite H7 in H); discriminate);
   try(apply IHceval in H8; inversion H8; 
   try (auto; split; reflexivity);
    try( rewrite <- H0 in H10; apply IHceval2 in H10;
        inversion H10); try(discriminate)); 
    try(apply IHceval1 in H8; inversion H8;
    try(try(rewrite <- H0 in H10); try(rewrite H0 in H10); apply IHceval2 in H10;
        inversion H10; try(rewrite H3; split; reflexivity); try(discriminate)));
        try(apply IHceval in H5; inversion H5; try(discriminate));
   try(apply IHceval in H9; try(assumption); try(inversion H9; try(discriminate);
   try(rewrite H0 in H; rewrite H10 in H; discriminate)));
   try(apply IHceval in H6; inversion H6; rewrite H0 in H; rewrite H10 in H;
   discriminate); try(apply IHceval1 in H8; inversion H8;
   try(rewrite H0 in H; rewrite H9 in H); try(discriminate));
   try(apply IHceval1 in H5; inversion H5; rewrite <- H0 in H10;
    apply IHceval2 in H10; inversion H10; try( rewrite <- H3 in H11;
    apply IHceval3 in H11; inversion H11); try (discriminate));
    try(assumption); try(inversion H11; discriminate);
    try( rewrite H0 in H; rewrite H9 in H; discriminate); try(discriminate);
    try(rewrite <-  H0 in H10; apply IHceval2 in H10; try(split; apply H10; reflexivity);
    try(inversion H10; discriminate)).
    

      + (* seq break 2 *) apply IHceval in H3. inversion H3. discriminate.
      + (* seq continue 1 *) apply IHceval1 in H5. inversion H5. discriminate.
      + (* seq continue 2 *) apply IHceval2. apply IHceval1 in H1. inversion H1;
         try (apply IHceval; assumption). rewrite H. assumption.
      +  apply IHceval1 in H4. inversion H4. apply IHceval2.
        rewrite <- H0 in H8. assumption.
      + apply IHceval1 in H7. inversion H7. discriminate.
...
(* Updated the [ceval] definition and fixed the proofs.
